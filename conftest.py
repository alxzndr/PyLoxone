...
Requirements (a failure to install here is not a test failure):
  * CPython >= 3.14.2: the Home Assistant release pinned by
    ``pytest-homeassistant-custom-component`` (see ``requirements-dev.txt``)
    refuses to import on older Pythons; see WP-0.1 / CORE-25.
...
Per-package fixtures live in ``tests/conftest.py``.
"""
