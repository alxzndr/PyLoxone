# Agent prompt — WP-6.9: Zeroconf discovery via `discover.py` if not done in WP-3.4.

You are implementing work package **WP-6.9** of the PyLoxone remediation plan.
...
### WP-6.9 Zeroconf discovery via `discover.py` if not done in WP-3.4.
- **Scope**: Zeroconf discovery via `discover.py` if not done in WP-3.4.
