# Agent prompt — WP-6.6: `InfoOnlyText`, `UpDownDigital`, `Tracker`; recursive `get_all` over `subControls`.

You are implementing work package **WP-6.6** of the PyLoxone remediation plan.
...
### WP-6.6 `InfoOnlyText`, `UpDownDigital`, `Tracker`; recursive `get_all` over `subControls`.
- **Scope**: `InfoOnlyText`, `UpDownDigital`, `Tracker`; recursive `get_all` over `subControls`.
